...
**多智能体协作管线（LangGraph 编排）**：采集 → 分析 → 校验 → 跨源关联 → 报告，
...
| 2. 分析 + 对抗评审 | Analyzer → Reviewer/Coordinator | 结构化威胁情报 + 评审记录 | 争议率、修复数、残留数、评审记录落库数 |
| 3. 校验 | Validator | 有效/无效判定，推送告警 | 有效 / 无效条数 |
| 4. 跨源关联 | Correlator | 聚合威胁事件 | 事件数、co-occurrence 印证率、置信度增益 |
| 5. 报告 | Reporter | Markdown 日报 | 报告字节数 |
...
├── src/                 # 核心源码
│   ├── agents/          # 感知/分析/校验/关联/报告/评审/协调 Agent
│   ├── graph/           # LangGraph 状态机工作流编排
│   ├── sources/         # 数据源适配器（含联想演示源 DemoSource）
│   ├── storage/         # 数据库模型与 CRUD
...
├── tests/               # 单元测试与集成测试
├── scripts/             # 运行脚本
└── data/                # 本地数据存储（含 benchmark 与评估结果）
```

## 合规声明

本项目仅使用公开 OSINT 数据源，用于学习、研究与安全防御目的。禁止用于非法爬取、未授权访问或任何违法行为。
